    ...
    def is_hit(self, x, y):
        ...
        for ship in self.ships:
            if (x, y) in ship.positions:
                ...
                self.board[y][x] = coloredString('RED', 'X')
                return True
        self.board[y][x] = coloredString('YELLOW', 'T')
        return False
...
def printBoards(player_board, computer_board):
    ...
    for i, row in enumerate(player_board.board):
        print(f'{coloredString('BLACK', " " + str(i + 1) + " " )}{'| '}{" | ".join(computer_board.board[i])}{' |'}        {coloredString('BLACK', " " + str(i + 1) + " ") }{'| '}{" | ".join(row)}{' |'}')
